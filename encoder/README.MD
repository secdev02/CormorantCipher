...
**Cormorant Cipher** is a document watermarking toolkit that embeds an invisible, machine-readable signature into typeset text. It works by patching an open-source typeface with a hidden second variant of every character — physically indistinguishable to a human reader, but detectable by software. When a watermarked document is opened, a DNS alert fires to notify the document owner.

It is intended for document security, leak detection, and honeypot scenarios — for example, tracking whether a confidential HTML report has been accessed or forwarded without authorisation.
...
### 3. Customise the secret string

Open `cormorant-cipher-encoder.html` in a browser. It runs entirely locally — nothing is transmitted. Type your secret and click **Encode**. It produces four copy-pasteable outputs:

| Output | Where to paste it in `cormorant-cipher.html` |
|---|---|
| `SECRET_BITS` array | `var SECRET_BITS = [...]` in the script block |
| `SECRET_TEXT` string | `var SECRET_TEXT = "..."` in the script block |
| `STEG_LABELS` array | `var STEG_LABELS = [...]` in the script block |
| Encoded HTML spans | Inner content of `#s1`, `#s2`, `#s3`, `#s-uc`, `#s-lc`, `#s-dg` |

Also update the `Payload` and `b32 subdomain` display rows in the tech panel to match.

The encoder validates that your carrier text is long enough — each character of your secret requires 8 glyphs of carrier text minimum (33-char secret = 264 chars needed). It will warn you if the secret is too long for the default specimen text.
...
cormorant-cipher-encoder.html  Browser-based tool to encode a custom secret
README.md                      This file
```
